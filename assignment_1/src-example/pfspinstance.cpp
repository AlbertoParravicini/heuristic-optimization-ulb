...
bool PfspInstance::readDataFromFile(char *fileName)
{
  ...
  if (fileIn.is_open())
  {
    ...
    for (j = 1; j <= nbJob; ++j)
    {
      for (m = 1; m <= nbMac; ++m)
      {
				...
				// We can discard the nth_machine value, as it's not useful!
  ...
  {
    cout << "ERROR. file:pfspInstance.cpp, method:readDataFromFile, " << "error while opening file " << fileName << std::endl;
...
long int PfspInstance::computeWCT(vector<int> &sol)
{
  ...
  // +1 as the first machine has 0 as end time of the previous one (which doesn't exist!)
